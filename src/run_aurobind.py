...
from aurobind.openfold.config import model_config
from aurobind.openfold.inference_config import get_model_config
from aurobind.openfold.model.model import AuroBind
from aurobind.openfold.utils.atom_token_conversion import aggregate_fn_advanced as aggregate_fn
from aurobind.openfold.model.confidences import get_summary_confidence, get_full_confidence

#### Data Processing
from aurobind.data.module.inference import get_inference_dataloader, construct_empty_template_features
from aurobind.data.types import MSA, Manifest, Record
from aurobind.data.write.writer import write_cif
from aurobind.data.inference.params import BoltzProcessedInput
from aurobind.data.inference.utils import download, get_cache_path
from aurobind.data.inference.data_tools import check_inputs, compute_msa, process_inputs, check_outputs, compute_similar_sequence
...
def predict_and_save(
    ...
    affinity = outputs['affinity_logits'].cpu()
    ...
    outname = f"{record.id}_seed-{seed}_affinity.json"
    output_path = struct_dir / outname
    with output_path.open("w") as f:
        json.dump(affinity_dict, f, indent=1)
...
def main(args):
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    model = AuroBind(config,generator = generator)
